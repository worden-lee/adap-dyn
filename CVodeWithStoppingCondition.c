/* This is a modified copy of part of cvode.c from the
   libsundials-serial-dev package.  It includes a modified CVode()
   function called CVodeWithStoppingCondition(), which uses a
   user-supplied function called stoppingCondition() to tell whether
   to stop integrating before the fixed stopping time is reached.

   June 13, 2007 by Lee Worden, from an earlier version that worked
   with an earlier version of CVODE.
*/

/*
 * -----------------------------------------------------------------
 * $Revision: 1.1 $
 * $Date: 2007-06-14 04:09:44 $
 * -----------------------------------------------------------------
 ...
 * -----
 *
 * stopping_condition() returns nonzero if it is time to stop
 * integrating and return, even though the stopping time tout has not
 * been reached.
 */
   ...
   int (*stopping_condition)(N_Vector, realtype, void *))
{
  ...
  loop {
    ...
    /* also do this if stopping_condition() says to stop integrating */

    //if (task == CV_ONE_STEP) {
    if (task == CV_ONE_STEP || stopping_condition(zn[0],tn,f_data)) {
      istate = CV_SUCCESS;
      tretlast = *tret = tn;
      N_VScale(ONE, zn[0], yout);
      next_q = qprime;
